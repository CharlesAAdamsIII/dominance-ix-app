...
async function realtimeGA4(token,property,startMinutesAgo,endMinutesAgo){const body={dimensions:[{name:'city'},{name:'region'},{name:'country'}],metrics:[{name:'activeUsers'},{name:'eventCount'},{name:'screenPageViews'}],minuteRanges:[{name:'window',startMinutesAgo,endMinutesAgo}],limit:'1000'};return googleJson(`https://analyticsdata.googleapis.com/v1beta/properties/${encodeURIComponent(property)}:runRealtimeReport`,token,{method:'POST',body:JSON.stringify(body)})}
function rowsToGeo(report){const out=new Map();for(const r of report.rows||[]){const city=r.dimensionValues?.[0]?.value||'(not set)',region=r.dimensionValues?.[1]?.value||'',country=r.dimensionValues?.[2]?.value||'';if(!city||city==='(not set)'||country!=='United States')continue;const active=Number(r.metricValues?.[0]?.value||0),events=Number(r.metricValues?.[1]?.value||0),views=Number(r.metricValues?.[2]?.value||0),key=(city+'|'+region).toLowerCase();out.set(key,{key,name:`${city}${region?', '+region:''}`,city,region,country,active,events,views})}return out}
...
async function buildMarketRadarSnapshot(a,connection,token,resource){
 ...
 for(const [key,g] of now){
   ...
   const metrics={active_users:g.active,event_count:g.events,page_views:g.views,prior_active_users:prev.active,source_note:'Near-real-time first-party GA4 activity. This is not national search volume.',search_volume_available:false,window_minutes:15};
