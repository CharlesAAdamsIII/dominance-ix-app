...
function stateFor(c,s){const resources=Array.isArray(c.metadata?.resources)?c.metadata.resources:[],selected=c.metadata?.selected_resource||null;if(c.status!=='connected')return'not_connected';if(!selected&&resources.length>1)return'resource_selection_required';if(c.last_error)return'error';if(s?.collected_at)return'synced';if(selected)return'awaiting_first_sync';return resources.length===1?'awaiting_first_sync':'discovering'}
function attach(router){
 router.get('/api/source-health/google',auth,async(req,res)=>{try{
  ...
  res.json({ok:true,account:a,sources,ga4_realtime:{observations_24h:Number(market.rows[0]?.n||0),last_observation_at:market.rows[0]?.last_at||null},latest_worker_scan:scan.rows[0]||null});
