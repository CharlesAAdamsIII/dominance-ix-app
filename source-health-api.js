...
function attach(router){
 router.get('/api/source-health/google',auth,async(req,res)=>{try{
  ...
  const [connections,snapshots,counts,market30d,scan]=await Promise.all([
   ...
   pool.query(`SELECT COUNT(*)::int n,MAX(captured_at) last_at FROM dominance_market_snapshots WHERE account_id=$1 AND source_key='GA4_30D' AND captured_at>NOW()-INTERVAL '24 hours'`,[a.id]).catch(()=>({rows:[{n:0,last_at:null}]})),
  ...
  res.json({ok:true,account:a,sources,google_ads_auth:{login_customer_id_configured:/^\d+$/.test(String(process.env.GOOGLE_ADS_LOGIN_CUSTOMER_ID||'').replace(/\D/g,'')),access_model:'google_cloud_project',developer_token_required:false},ga4_market_30d:{observations_24h:Number(market30d.rows[0]?.n||0),last_observation_at:market30d.rows[0]?.last_at||null,window_days:30,comparison_days:30},latest_worker_scan:scan.rows[0]||null});
