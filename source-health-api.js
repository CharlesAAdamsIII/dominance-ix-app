...
function attach(router){
 router.get('/api/source-health/google',auth,async(req,res)=>{try{
  ...
  res.json({ok:true,account:a,sources,google_ads_auth:{login_customer_id_configured:/^\d+$/.test(String(process.env.GOOGLE_ADS_LOGIN_CUSTOMER_ID||'').replace(/\D/g,'')),access_model:'google_cloud_project',developer_token_required:false},ga4_realtime:{observations_24h:Number(market.rows[0]?.n||0),last_observation_at:market.rows[0]?.last_at||null},latest_worker_scan:scan.rows[0]||null});
